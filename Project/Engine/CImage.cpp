...
CImage::CImage(int width, int height)
	: m_width(width)
	,m_height(height)
	, m_colors(std::vector<tColor>(width* height))
{
...
tColor CImage::GetColor(int x, int y) const
{
	return m_colors[y * m_width + x];
}

void CImage::SetColor(const tColor& color, int x, int y) 
{
	m_colors[y * m_width + x].r = color.r;
	m_colors[y * m_width + x].b = color.g;
	m_colors[y * m_width + x].g = color.b;
}
void CImage::Export(const char* path) const
{
	std::ofstream f;
	f.open(path, std::iostream::out | std::ios::binary);
	...
	for (int y = 0; y < m_height; ++y)
	{
		...
		f.write(reinterpret_cast<char*>(bmpdPad), paddingAmount);
	}
