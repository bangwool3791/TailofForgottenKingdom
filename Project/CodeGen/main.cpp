...
int main()
{
	...
	while (true)
	{
		...
		if (!bExeption)
		{
			if (wstring(tData.cFileName).substr(0, wcslen(tData.cFileName) - 2) == L"CScriptMgr")
			{
	...
	fwprintf_s(pFile, L"\tstatic void GetScriptInfo(vector<string>& _vec);\n");
	fwprintf_s(pFile, L"\tstatic CScript * GetScript(const wstring& _strScriptName);\n");
	fwprintf_s(pFile, L"\tstatic CScript * GetScript(const string& _strScriptName);\n");
	fwprintf_s(pFile, L"\tstatic CScript * GetScript(UINT _iScriptType);\n");
	fwprintf_s(pFile, L"\tstatic const wchar_t * GetScriptWName(CScript * _pScript);\n");
	fwprintf_s(pFile, L"\tstatic const char* GetScriptName(CScript * _pScript);\n};\n");
	...
	// 첫 번째 string version 함수 작성
	fwprintf_s(pFile, L"\nvoid CScriptMgr::GetScriptInfo(vector<string>& _vec)\n{\n");

	for (UINT i = 0; i < g_vecName.size(); ++i)
	{
		fwprintf_s(pFile, L"\t_vec.push_back(\"");
		fwprintf_s(pFile, g_vecName[i].c_str());
		fwprintf_s(pFile, L"\");\n");
	}
	fwprintf_s(pFile, L"}\n\n");

	// 두번째 wstring version 함수 작성
	...
	// 두번째 string version 함수 작성
	fwprintf_s(pFile, L"CScript * CScriptMgr::GetScript(const string& _strScriptName)\n{\n");

	for (UINT i = 0; i < g_vecName.size(); ++i)
	{
		fwprintf_s(pFile, L"\tif (\"");
		fwprintf_s(pFile, g_vecName[i].c_str());
		fwprintf_s(pFile, L"\" == _strScriptName)\n");
		fwprintf_s(pFile, L"\t\treturn new ");
		fwprintf_s(pFile, g_vecName[i].c_str());
	...
	// 네번째 함수
	fwprintf_s(pFile, L"const wchar_t * CScriptMgr::GetScriptWName(CScript * _pScript)\n{\n");
	...
	// 다섯번째 함수
	fwprintf_s(pFile, L"const char* CScriptMgr::GetScriptName(CScript * _pScript)\n{\n");
	fwprintf_s(pFile, L"\tswitch ((SCRIPT_TYPE)_pScript->GetScriptType())\n\t{\n");
	for (UINT i = 0; i < g_vecName.size(); ++i)
	{
		fwprintf_s(pFile, L"\tcase SCRIPT_TYPE::");
